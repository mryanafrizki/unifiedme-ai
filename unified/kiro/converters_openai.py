...
log = logging.getLogger("unified.kiro.converters_openai")

from .model_resolver import get_model_id_for_kiro
from .models_openai import ChatMessage, ChatCompletionRequest, Tool
...
def _extract_images_from_tool_message(content: Any) -> List[Dict[str, Any]]:
    ...
    if images:
        log.debug(f"Extracted {len(images)} image(s) from tool message content")
...
def convert_openai_tools_to_unified(tools: Optional[List[Tool]]) -> Optional[List[UnifiedTool]]:
    ...
    for tool in tools:
        ...
        # Skip invalid tools
            ...
            log.warning(f"Skipping invalid tool: no function or name field found")
            continue
