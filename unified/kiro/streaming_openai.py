...
log = logging.getLogger("unified.kiro.streaming_openai")

from .parsers import parse_bracket_tool_calls, deduplicate_tool_calls
from .utils import generate_completion_id
from .config import (
    FIRST_TOKEN_TIMEOUT,
    FIRST_TOKEN_MAX_RETRIES,
    FAKE_REASONING_HANDLING,
)
# Simplified token counting — tiktoken optional
try:
    import tiktoken
    ...
    def count_tokens(text: str) -> int:
        return len(text) // 4 if text else 0  # rough estimate
    ...
    try:
        ...
        async for event in parse_kiro_stream(response, first_token_timeout):
            if event.type == "content" and event.content:
                ...
                # WebSearch MCP interception not ported — pass through as normal tool call
                if False:  # web_search interception disabled
                    
                    log.info("Intercepted web_search tool call (Path B - MCP emulation)")
                    ...
                    if not query:
                        log.warning("web_search called without query, skipping MCP call")
                        # Continue with normal tool_use processing
                    else:
                        log.debug(f"WebSearch query (Path B): {query}")
                        
                        # Call MCP API
                        mcp_tool_use_id, results = await call_kiro_mcp_api(query, auth_manager)
                        
                        if results is None:
                            log.error("MCP API call failed for web_search")
        ...
        if content_was_truncated:
            from .config import TRUNCATION_RECOVERY
            log.error(
        ...
        prompt_tokens, total_tokens, prompt_source, total_source = calculate_tokens_from_context_usage(
            context_usage_percentage, completion_tokens
        )
        ...
        if prompt_source == "unknown" and request_messages:
            # Estimate prompt tokens from messages (no tiktoken for messages)
            import json as _json
            prompt_tokens = len(_json.dumps(request_messages, ensure_ascii=False)) // 4
            if request_tools:
                prompt_tokens += len(_json.dumps(request_tools, ensure_ascii=False)) // 4
        ...
        if all_tool_calls:
            log.debug(f"Processing {len(all_tool_calls)} tool calls for streaming response")
            ...
            for idx, tc in enumerate(all_tool_calls):
                ...
                log.debug(f"Tool call [{idx}] '{tool_name}': id={tc.get('id')}, args_length={len(tool_args)}")
        ...
        # Truncation recovery not ported
        ...
        try:
            ...
            log.debug(f"Error closing response: {close_error}")
        
        if streaming_error_occurred:
            log.debug("Streaming completed with error")
        else:
...
async def stream_kiro_to_openai(
    ...
    async for chunk in stream_kiro_to_openai_internal(
        client, response, model, auth_manager,
