...
log = logging.getLogger("unified.kiro.converters_core")
...
from .payload_guards import check_payload_size, trim_payload_to_limit
...
def extract_images_from_content(content: Any) -> List[Dict[str, Any]]:
    ...
    for item in content:
        ...
        if item_type == "image_url":
            ...
            if url.startswith("data:"):
                ...
                log.warning(f"URL-based images are not supported by Kiro API, skipping: {url[:80]}...")
    ...
    if images:
        log.debug(f"Extracted {len(images)} image(s) from content")
...
def inject_thinking_tags(content: str, thinking_config: ThinkingConfig) -> str:
    ...
    if not thinking_config.enabled:
        log.debug("Thinking disabled by client request")
        return content
    ...
    log.debug(f"Injecting thinking tags with budget={effective_budget}")
...
def convert_tools_to_kiro_format(tools: Optional[List[UnifiedTool]]) -> List[Dict[str, Any]]:
    ...
    for tool in tools:
        ...
        if not description or not description.strip():
            ...
            log.debug(f"Tool '{tool.name}' has empty description, using placeholder")
...
def convert_images_to_kiro_format(images: Optional[List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
    ...
    for img in images:
        ...
        if not data:
            log.warning("Skipping image with empty data")
            continue
        ...
        if data.startswith("data:"):
            try:
                ...
                data = actual_data
                log.debug(f"Stripped data URL prefix, extracted media_type: {media_type}")
            except (ValueError, IndexError) as e:
                log.warning(f"Failed to parse data URL prefix: {e}")
    ...
    if kiro_images:
        log.debug(f"Converted {len(kiro_images)} image(s) to Kiro format")
...
def merge_adjacent_messages(messages: List[UnifiedMessage]) -> List[UnifiedMessage]:
    ...
    if total_merges > 0:
        ...
        if extras:
            log.debug(f"Merged {total_merges} adjacent messages ({merge_summary}), including {', '.join(extras)}")
        else:
            log.debug(f"Merged {total_merges} adjacent messages ({merge_summary})")
...
def normalize_message_roles(messages: List[UnifiedMessage]) -> List[UnifiedMessage]:
    ...
    for msg in messages:
        if msg.role not in ("user", "assistant"):
            log.debug(f"Normalizing role '{msg.role}' to 'user'")
    ...
    if converted_count > 0:
        log.debug(f"Normalized {converted_count} message(s) with unknown roles to 'user'")
...
def ensure_alternating_roles(messages: List[UnifiedMessage]) -> List[UnifiedMessage]:
    ...
    if synthetic_count > 0:
        log.debug(f"Inserted {synthetic_count} synthetic assistant message(s) to ensure alternation")
...
def build_kiro_payload(
    ...
    if images:
        kiro_images = convert_images_to_kiro_format(images)
        if kiro_images:
            log.debug(f"Added {len(kiro_images)} image(s) to current message")
